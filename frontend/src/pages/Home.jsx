    ...
    <div className="relative min-h-screen bg-[#110A08] text-white font-sans overflow-hidden flex flex-col items-center transition-colors duration-1000">
      ...
      <nav className="relative z-50 w-full max-w-7xl flex justify-between items-center p-8">
        ...
        <button 
          ...
          className="p-2 bg-white/5 rounded-full hover:bg-white/10 transition border border-white/10 backdrop-blur-sm relative z-50">
      ...
      <section className="relative z-50 w-full max-w-6xl mt-16 mb-12 flex items-center justify-center px-12">
        ...
        <div className="relative w-full max-w-4xl h-[450px] shadow-2xl group overflow-hidden transition-all duration-500">
          ...
          <div className="absolute bottom-6 left-16 z-40 flex gap-8 text-xs font-bold tracking-[0.15em] text-white uppercase">
            <Link to={tarjetaVisible.ruta} className="hover:text-black transition-colors flex items-center gap-2 group/btn bg-white/0 hover:bg-white px-3 py-2 -ml-3 rounded-full">
      ...
      {menuAbierto && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex flex-col justify-center items-center">
          
          {/* Botón para cerrar (X) */}
          <button 
            onClick={() => setMenuAbierto(false)}
            className="absolute top-10 right-10 text-white/50 hover:text-white text-4xl font-light transition-colors"
          >
            ×
          </button>
          ...
          <nav className="flex flex-col items-center gap-8 text-2xl font-light tracking-widest uppercase">
            <Link to="/tienda/impresoras" className="hover:text-[#E87A5D] transition-colors" onClick={() => setMenuAbierto(false)}>Impresoras 3D</Link>
            <Link to="/tienda/filamentos" className="hover:text-[#E87A5D] transition-colors" onClick={() => setMenuAbierto(false)}>Filamentos</Link>
            <Link to="/tienda/accesorios" className="hover:text-[#E87A5D] transition-colors" onClick={() => setMenuAbierto(false)}>Accesorios</Link>
            
            <div className="w-12 h-px bg-white/20 my-4"></div> {/* Separador */}
            
            <Link to="/soporte" className="text-[#E87A5D] font-bold hover:text-white transition-colors flex items-center gap-3" onClick={() => setMenuAbierto(false)}>
              <span>Soporte IA</span> <span className="text-sm bg-[#E87A5D]/20 px-2 py-1 rounded">Beta</span>
            </Link>
          </nav>
